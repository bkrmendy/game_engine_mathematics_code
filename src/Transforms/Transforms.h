...
{
    return Matrix<double, 2, 2>{ { std::cos(angle), -std::sin(angle), std::sin(angle), std::cos(angle) } };
}

Matrix<double, 3, 3> Rotation3Z(Utility::Radian angle)
{
    return Matrix<double, 3, 3>{ { std::cos(angle), -std::sin(angle), 0, std::sin(angle), std::cos(angle), 0, 0, 0, 1 } };
}
...
Matrix<double, 3, 3> Rotation3(Vector<3, double> axis, Utility::Radian angle)
{
    ...
    return Matrix<double, 3, 3>{ { a, b, cc, d, e, f, g, h, i } };
}
